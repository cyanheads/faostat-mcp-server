...
**Schema (per-domain table):** `area_code INTEGER, area_m49 TEXT, area TEXT, item_code INTEGER, item TEXT, element_code INTEGER, element TEXT, year INTEGER, unit TEXT, value DOUBLE, flag TEXT, note TEXT`. Indexes on `(area_code)`, `(item_code)`, `(element_code)`, `(year)`, plus the composites `(element_code, year)` and `(item_code, element_code, year)` matching the common filter + `ORDER BY year` shapes. FTS5 over `area`, `item`, `element` (drives `resolve_codes`). The ingester maps columns off the **actual CSV header per domain** (not a hardcoded order) so a non-standard domain fails loudly rather than mis-mapping.

**Query-planner statistics.** The composite indexes only pay off once SQLite has cardinality statistics. With no `sqlite_stat1`, the cost-based optimizer cannot tell the two composites apart and seeks `(element_code, year)` for an item+element filter, scanning that entire element slice: measured warm on the real `QCL` mirror (4.2M rows), ~1,190 ms vs ~6 ms for the year-scoped ranking `faostat_commodity_profile` runs, with `EXPLAIN QUERY PLAN` confirming the index switch. A cold first touch of the 916 MB file is worse again. The penalty is largest exactly where a year bound narrows an item+element filter — the aggregate shapes below. The mirror therefore runs a full `ANALYZE` per cube table — lazily on the first read when the file carries no statistics (so a mirror synced before this existed is caught up without a re-sync), and again after any sync that applies rows, since a domain ZIP replaces the vintage the previous statistics described. A sampled `ANALYZE` (`PRAGMA analysis_limit`) is not a substitute: every limit measured produced uniform per-key estimates that left the wrong index selected. The pass is deliberately never run at startup — a few seconds on `QCL`, ~49 s on `TCL` — so it cannot stall the process before it can serve `/healthz`.

The lazy gate sits on the reads the statistics serve — the filtering and aggregating paths — and not on `resolve_codes`'s distinct-dimension-code scan, which the single-column index already satisfies: charging that call, the one an agent makes first, for an `ANALYZE` it cannot use turns the cheapest request into the slowest one. Statistics are an optimization, so a failed `ANALYZE` (read-only volume, a writer holding the lock past `busy_timeout`) degrades to the uninformed plan rather than failing the query, and only the outcome is remembered — a later read retries, so contention that clears costs one slow query instead of every query for the process's remaining life, bounded at three attempts per domain so an obstruction that never clears cannot re-run a 49-second pass on every request.
